// Frontend form validation + integration with backend POST endpoint
(function () {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('/submit_request', {
        ...
        body: JSON.stringify({ subject: subject.value.trim(), description: description.value.trim() })
      });
      ...
      if (res.ok && data.status === 'received') {
        ...
        success.hidden = false;
        success.textContent = data.error || 'Server returned an unexpected response.';
      }
    } catch (err) {
      success.hidden = false;
